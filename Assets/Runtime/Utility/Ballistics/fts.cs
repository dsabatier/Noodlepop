...
namespace Noodlepop
{
	public class Ballistics
	{
		...
		/// <summary>
		/// Provided an angle, gravity force, start position and target figure out the force needed to launch
		/// </summary>
		...
		/// <returns></returns>
		private Vector3 GetVelocityToHitTarget2D(float angle, float gravity, Vector2 startPosition, Vector2 targetPosition)
		{
			float radAngle = Mathf.Deg2Rad * angle;
			float distance = Vector2.Distance(startPosition, targetPosition); // distance between targets
			float yOffset = startPosition.y - targetPosition.y; // difference in height
        
			float initialVelocity = (1 / Mathf.Cos(radAngle)) *
			                        Mathf.Sqrt(0.5f * gravity * Mathf.Pow(distance, 2) /
			                                   (distance * Mathf.Tan(radAngle) + yOffset));

			// the velocity, not rotated towards the target
			Vector2 velocity = new Vector2(initialVelocity * Mathf.Sin(radAngle), initialVelocity * Mathf.Cos(radAngle));
        
			// rotate towards the target
			float angleBetweenObjects = Vector2.Angle(Vector2.right, targetPosition - startPosition);
		...
		// Returns number of solutions.
		public static int SolveQuartic(double c0, double c1, double c2, double c3, double c4, out double s0,
		...
		public static float ballistic_range(float speed, float gravity, float initial_height)
		{
			...
			Debug.Assert(speed > 0 && gravity > 0 && initial_height >= 0,
				"Ballistics.ballistic_range called with invalid data");
		...
		public static int solve_ballistic_arc(Vector3 proj_pos, float proj_speed, Vector3 target, float gravity,
			out Vector3 lowAngle, out Vector3 highAngle)
		{

			// Handling these cases is up to your project's coding standards
			Debug.Assert(proj_pos != target && proj_speed > 0 && gravity > 0,
				"Ballistics.solve_ballistic_arc called with invalid data");

			// C# requires out variables be set
			lowAngle = Vector3.zero;
			highAngle = Vector3.zero;
			...
			lowAngle = groundDir * Mathf.Cos(lowAng) * proj_speed + Vector3.up * Mathf.Sin(lowAng) * proj_speed;
			if (numSolutions > 1)
				highAngle = groundDir * Mathf.Cos(highAng) * proj_speed + Vector3.up * Mathf.Sin(highAng) * proj_speed;
			...
			out Vector3 impact_point)
		{
			...
			Debug.Assert(proj_pos != target && lateral_speed > 0,
				"Ballistics.solve_ballistic_arc_lateral called with invalid data");
